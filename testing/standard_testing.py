...
def test_solver(solver, test_dataloader, measurement_process, device="cpu"):
    #####################TEST##########################
    loss_accumulator = []
    ...
    print(
        "TEST LOSS: " + str(sum(loss_accumulator) / len(loss_accumulator)), flush=True
    )
    print("MEAN TEST PSNR: " + str(PSNR), flush=True)
    print(
        "TEST PSNR QUARTILES AND MEDIAN: "
        + str(percentiles[0])
        + ", "
        + str(percentiles[1])
        + ", "
