...
def swap_patches(batch, index1, index2, h, w, patch_top_loc, patch_left_loc):
    tmp = batch[
        ...
        patch_left_loc : patch_left_loc + w,
        :,
    ].clone()
...
def batch_patch_swap(
    ...
    for index in range(num_images):
        ...
        batch = swap_patches(
            batch, index, swap_index, h, w, patch_top_loc, patch_left_loc
        )
...
def directory_filelist(target_directory):
    file_list = [
        f
        for f in sorted(os.listdir(target_directory))
...
class FolderDataset(Dataset):
    def __init__(self, target_directory, transform=None):
        ...
        self.full_filelist = [
            target_directory + single_file for single_file in filelist
        ]
...
class CelebaDataset(Dataset):
    def __init__(self, target_directory, validation_data=False, transform=None):
        ...
        if validation_data:
            self.full_filelist = [
                target_directory + single_file for single_file in val_data
            ]
        else:
            self.full_filelist = [
                target_directory + single_file for single_file in training_data
            ]
...
class CelebaTestDataset(Dataset):
    def __init__(self, target_directory, transform=None):
        ...
        test_data = filelist[182638:]
        self.full_filelist = [
            target_directory + single_file for single_file in test_data
        ]
        self.transform = transform
